...
export default function GameRoom({ player, memberId, roomId, onExit }) {
  ...
  // Moves cards between piles
  function moveCardTo(targetZone) {
    if (!selectedCard) return;
    pushUndo();

    // Remove from whichever zone it is in
    const newHand = {
    ...
    // Add to target zone if bounds permit
          ...
          <div className="my-hand safe-area-bottom">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <span style={{ fontSize: '11px', color: 'var(--text-muted)' }}>ชิป: {Math.round(chips * 10) / 10}</span>
              </div>
            </div>

            {/* Click-to-move Piles zones */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '8px' }}>
              <div className="hand-pile-container">
                ...
                >
                  {hand.back.map((c, i) => (
                    <div 
                      key={i} 
                      className={`poker-card ${selectedCard === c ? 'glow-bonus' : ''} ${c.suit === '♥' || c.suit === '♦' ? 'red-card' : 'black-card'}`}
                      onClick={(e) => { e.stopPropagation(); selectCard(c); }}
                    >
                      <span className="card-num">{c.rank}</span>
                      <span className="card-suit">{c.suit}</span>
                    </div>
                  ))}
                  {hand.back.length === 0 && <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.15)', alignSelf: 'center', margin: 'auto' }}>จิ้มเพื่อจัดกองหลัง</span>}
                </div>
                {hand.back.length === 5 && (
              ...
              <div className="hand-pile-container">
                ...
                >
                  ...
                  {hand.mid.length === 0 && <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.15)', alignSelf: 'center', margin: 'auto' }}>จิ้มเพื่อจัดกองกลาง</span>}
                </div>
                {hand.mid.length === 5 && (
                  <span style={{ alignSelf: 'center', fontSize: '9px', background: 'var(--glass)', border: '1px solid var(--line)', padding: '2px 4px', borderRadius: '4px', color: 'var(--primary)' }}>
              ...
              <div className="hand-pile-container">
                ...
                >
                  ...
                  {hand.front.length === 0 && <span style={{ fontSize: '11px', color: 'rgba(255,255,255,0.15)', alignSelf: 'center', margin: 'auto' }}>จิ้มเพื่อจัดกองหน้า</span>}
                </div>
                {hand.front.length === 3 && (
                  <span style={{ alignSelf: 'center', fontSize: '9px', background: 'var(--glass)', border: '1px solid var(--line)', padding: '2px 4px', borderRadius: '4px', color: 'var(--primary)' }}>
            ...
            <div style={{ marginBottom: '8px' }}>
              <div 
                className="drop-zone" 
                style={{ minHeight: '66px', display: 'flex', gap: '3px' }}
                onClick={() => moveCardTo('unplaced')}
              >
                {hand.unplaced.map((c, i) => (
                  <div 
                    key={i} 
                    className={`poker-card ${selectedCard === c ? 'glow-bonus' : ''} ${c.suit === '♥' || c.suit === '♦' ? 'red-card' : 'black-card'}`}
                    onClick={(e) => { e.stopPropagation(); selectCard(c); }}
            ...
            {calcScores(players.filter(p => !p.isSpectator), room.hands || {}).map((s, idx) => {
              ...
              return (
                <div key={idx} className="glass-panel animate-pop-up" style={{ padding: '12px', borderLeft: '4px solid ' + (isWin ? '#40e880' : s.roundScore < 0 ? '#ff6d86' : 'var(--line)') }}>
                  <div style={{ display: 'flex', justify: 'space-between', fontWeight: '800', fontSize: '14px' }}>
